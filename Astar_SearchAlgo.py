...
def AStarSearch():
    ...
    closed = []  # closed nodes
    opened = [['S', 8]]  # opened nodes         # the f(S) = 0 + 8 :  here 8 is heuristic cost

    '''find the visited nodes'''
    while True:
        fn = [i[1] for i in opened]  # fn = f(n) = g(n) + h(n) : ths 'fn' is list
        chosen_index = fn.index(min(fn))  # taking the min value from the list
        node = opened[chosen_index][0]  # current node
        closed.append(opened[chosen_index])
        del opened[chosen_index]
        if closed[-1][
            0] == 'G':  # break the loop if node G has been found, end of the array so -1 and picking the first parameter so G
            break
